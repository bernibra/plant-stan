...
# - Binomial regression with gaussian RBFs - faster version - with traits
####
...
binomial.stan.gauss.RBFs.traits <- function(d = NULL, recompile = T, simulated=T, min.occurrence=10, ofolder="../../results/models/"){
        # Fixing some of the options
        variables=c("bio5_", "bio6_","bio12_", "gdd5_", "bio1_","bio15_","bio17_", "bio8_", "TabsY_")
        gp_type <- 2
        ndim <- 2
        ...
        saveRDS(mfit_5.1, file = paste(ofolder, "binomial-stan-gauss-RBFs-traits",extension,".rds", sep=""))
        return(mfit_5.1)
}

binomial.stan.gauss.RBFs.traits_beta <- function(d = NULL, recompile = T, simulated=T, min.occurrence=10, ofolder="../../results/models/"){
        # Fixing some of the options
        variables=c("bio5_", "bio6_","bio12_", "gdd5_", "bio1_","bio15_","bio17_", "bio8_", "TabsY_")
        gp_type <- 2
        ndim <- 2
        ...
        start_5.1 <- list(
                ...
                zetasq_a = rep(0, 3),
                zrhosq_a = rep(0, 3),
                zsigma_b = rep(0, dat_5.1$K),
                zetasq_b = matrix(0, dat_5.1$K, 3),
                zrhosq_b = matrix(0, dat_5.1$K, 3),
                zsigma_g = rep(0, dat_5.1$K),
                zetasq_g = matrix(0, dat_5.1$K, 3),
                zrhosq_g = matrix(0, dat_5.1$K, 3),
                sigma_sigma = 0.1,
                sigma_mean = 0,
                rho_sigma = 0.1,
                rho_mean = 0,
...
binomial.stan.gauss.RBFs.traits_beta(simulated=F, recompile = F, ofolder="/cluster/scratch/bemora/plant-stan/")
