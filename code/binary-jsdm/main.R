...
# This first one is just a logistic regression, all "variables" are used as predictors in a linear form.
binomial.stan.gauss.RBFs2 <- function(d = NULL, variables=c("bio5_", "bio6_","bio12_"), recompile = T, loglik=F, show.plot=T, pca=T, ndim=2, simulated=T, ofolder="../../results/models/"){
        ...
        start_3.1 <- list(
                ...
                alpha_bar = 0,
                sigma_a = 0.1,
                sigma_zbeta = matrix(0.1, dat_3.1$K, dat_3.1$L),
                zbeta = matrix(0, dat_3.1$K, dat_3.1$L),
                sigma_beta_bar = rep(0.1, dat_3.1$K),
                beta_bar = rep(0, dat_3.1$K),
                sigma_b = rep(0.1, dat_3.1$K),
                etasq = rep(0.1, dat_3.1$K),
                rhosq = rep(0.1, dat_3.1$K),
                sigma_b_sigma = rep(0.1, dat_3.1$K),
                etasq_sigma = rep(0.1, dat_3.1$K),
                rhosq_sigma = rep(0.1, dat_3.1$K)
        )
        ...
        saveRDS(mfit_3.1, file = paste(ofolder, "binomial-stan-gauss-RBFs",extension,"2.rds", sep=""))
        return(mfit_3.1)
}
