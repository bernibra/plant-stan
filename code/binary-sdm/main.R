...
# - Zero-inflated regression with gaussian RBFs.
####
...
# This final model combine that zeroinflated approach with the use of gaussian RBFs
# NOTE: Same as above, while I like this option, it does not do very well. I am not sure why... but I like it for incorporating information about traits.
binomial.stan.gaussian.zeroinflated <- function(d=NULL, idx=128, variables=c("bio5_", "bio6_","bio12_"), pseudoA=F, loglik=F, show.plot=T, ofolder="../../results/models/"){
        if(pseudoA){
                warning("This will take quite some time to run.\n Unless you are running this on the cluster, I would think twice before wasting time on a model with stupid quadratic terms.")
        }
        ...
        n_chains_6.0 <- 3
        init_6.0 <- list()
        for ( i in 1:n_chains_6.0 ) init_6.0[[i]] <- start_6.0
        ...
        # Run stan model
        mfit_6.0 <- stan ( model_code=model_code ,
                           data=dat_6.0 ,
                           chains=n_chains_6.0 ,
                           cores= n_chains_6.0 ,
                           warmup=1000, iter=4000,
                           init=init_6.0 , control = list(adapt_delta = 0.95))
        
        # Build link function to make predictions        
        link_6.0 <- function(dat, post){
                p <- 0
                for(k in 1:dim(dat)[2]){
                        a <- as.matrix(dat[,k])
                        b <- t(post$mu_bar[,k])
                ...
                p <- inv.logit(t(p + as.matrix(post$alpha)[col(p)])) * inv.logit(t(p*0 + as.matrix(post$gamma)[col(p)]))
        ...
        # Sample from the posterior
        post <- extract.samples(mfit_6.0)
        
        # Make predictions with test data
        p_post <- link_6.0(post = post, dat=dat)
        p_mu <- apply( p_post , 2 , mean )
        p_ci <- apply( p_post , 2 , PI )
        ...
        out.object <- list(mfit = mfit_6.0, roc_obj = roc_obj, roc_maxent = d$roc_maxent, d=d)
        saveRDS(out.object, file = paste(ofolder, "sdm-binomial-gaussian.rds", sep=""))
        return(out.object)
}
