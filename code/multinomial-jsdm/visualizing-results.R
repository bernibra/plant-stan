...
plot.simulated.data <- function(beta=T, gp_type = 2){
  # load data
  d <- readRDS(file = paste("../../data/processed/jsdm/S1S2", "_abundance_data.rds", sep = ""))
  if(beta){
    model <- readRDS(paste("../../results/models/multinomial-stan-gauss-RBFs-beta-simulated",as.character(gp_type),".rds", sep=""))
  }else{
