...
prepare.data <- function(idx=1, variables = c("bio5_", "bio6_","bio12_")){
        ...
        obs.data$abundance <- as.numeric(as.factor(obs.data$abundance))
        absence.data <- data.frame(easting = obs.data$easting[!(presence)], northing = obs.data$northing[!(presence)], abundance = obs.data$abundance[!(presence)])        
        obs.data <- data.frame(easting = obs.data$easting[presence], northing = obs.data$northing[presence], abundance = obs.data$abundance[presence])
...
species_distribution.maxent <- function(idx=1, view_plots=T, variables=c("bio5_", "bio6_","bio12_"), pseudoA = F){
        ...
        # Run maxent
        bc.model <- dismo::maxent(dat$bioclim.data, p = presence.train[,c("easting", "northing")], a = absence.train[,c("easting", "northing")], removeDuplicates=TRUE) # Maxent uses all the environment and presense data, regularisation from the line above. removeDuplicates is just a precaution, it is not neccessary.

        # Also run maxent with pseudo-absences for comparison purposes
        if(!(pseudoA)){
           bc.model_ <- dismo::maxent(dat$bioclim.data, p = presence.train[,c("easting", "northing")], removeDuplicates=TRUE) # Maxent uses all the environment and presense data, regularisation from the line above. removeDuplicates is just a precaution, it is not neccessary.                
        }
        ...
        if(pseudoA){
                train.presence <- data.frame(obs=presence.train$abundance, train=1, bc.model@presence)
                train.absence <- data.frame(obs=absence.train$abundance[1], train=1, bc.model@absence)
                test.presence <- data.frame(obs=presence.test$abundance, train=0, raster::extract(dat$bioclim.data, presence.test[,c("easting", "northing")]))
                test.absence <- data.frame(obs=absence.test$abundance[1], train=0,raster::extract(dat$bioclim.data, absence.test[,c("easting", "northing")]))
        }else{
                train.presence <- data.frame(obs=presence.train$abundance, train=1, raster::extract(dat$bioclim.data, presence.train[,c("easting", "northing")]))
                train.absence <- data.frame(obs=absence.train$abundance, train=1, raster::extract(dat$bioclim.data, absence.train[,c("easting", "northing")]))
                test.presence <- data.frame(obs=presence.test$abundance, train=0, raster::extract(dat$bioclim.data, presence.test[,c("easting", "northing")]))
                test.absence <- data.frame(obs=absence.test$abundance, train=0,raster::extract(dat$bioclim.data, absence.test[,c("easting", "northing")]))
        }
