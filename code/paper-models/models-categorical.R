...
categorical.model.generror.1d.multithread <- "
functions{
    matrix cov_GPL2(matrix x, real a, real b, real delta) {
        int N = dims(x)[1];
        matrix[N, N] K;
...
# Binomial 1d
categorical.model.skew.1d.multithread <- "
functions{
    matrix cov_GPL2(matrix x, real a, real b, real delta) {
        int N = dims(x)[1];
        matrix[N, N] K;
...
categorical.model.skew.traits.1d.multithread <- "
functions{
    matrix cov_GPL2(matrix x, real a, real b, real delta) {
        int N = dims(x)[1];
        matrix[N, N] K;
...
parameters{
    ...
    real<lower=0> etasq_g;
    real<lower=0> rhosq_g;
    real<lower=0> etasq_l;
    real<lower=0> rhosq_l;
}
transformed parameters{
    vector[L] lambda;
    vector[L] alpha;
    ...
    matrix[L, L] L_SIGMA_b;
    matrix[L, L] L_SIGMA_g;
    matrix[L, L] L_SIGMA_l;

    alpha = exp(zalpha * sigma_a + alpha_bar);
    L_SIGMA_l = cholesky_decompose(cov_GPL2(Dmat_l, etasq_l, rhosq_l, sigma_l));
    lambda = inv_logit(L_SIGMA_l * zlambda + lambda_bar)*2-1;
